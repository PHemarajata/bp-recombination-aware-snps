...
**`Act4NotSeparable` HAS NO SOURCE.** It is listed above as a legacy part, but
`tuc_legacy_scenes_merged.py` holds only five scenes and that is not one of
them: `NegativeControlZoom`, `DetectionWindowSweep`, `RecursiveSubdivision`,
`TreeBuilderPaired`, `OutbreakThreshold`. The string `Act4NotSeparable` appears
in `tuc_clip4_scenes.py` only inside a comment, and
`git log --all -S "class Act4NotSeparable"` returns nothing, so it has never
been in this repository. The 51.0 s mp4 in clip 4's `parts/` is the only copy
and **it cannot be re-rendered**. Given the ANIMO_* briefs in this repo it was
most likely generated by Animo, whose scenes live in Animo's own storage.

That blocks a series-wide visual change: 51.0 s of clip 4's 472.6 s, about 11
percent of the film, cannot follow the rest. Whoever picks this up has to
choose between rebuilding the scene from its brief, recovering it from Animo,
filtering the existing mp4, or accepting the mismatch. **Decide this before
starting a palette rollout, not during one.**
