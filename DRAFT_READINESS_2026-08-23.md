...
| **pipeline release cited** | ✅ **tagged 2026-08-26**: `v1.0.5-mod` at `79ab645`, pushed. ⚠ Methods must state the run is **not seed-reproducible**: this pipeline never passes Gubbins' `--seed`, so Gubbins draws a random RAxML seed ⚠ **CORRECTED 2026-09-04: there is no `gubbins_seed` fix.** No such parameter or commit exists on any branch of the pipeline repository; PR #5 of 2026-08-19, the date it was attributed to, pins CSV line terminators. Gubbins 3.4.3 exposes `--seed` and this pipeline never passes it, so the zero-seed failure is live in current code, not confined to the pin. and can silently drop a unit while still exiting 0 |
