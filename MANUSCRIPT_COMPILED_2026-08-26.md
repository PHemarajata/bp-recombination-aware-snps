# Origin attribution in *Burkholderia pseudomallei*: compiled manuscript draft

**Compiled 2026-08-26** from the current section drafts, for review. This file is a
**compilation, not a new draft**: every word below is copied verbatim from the
source files listed under each section heading, with heading levels demoted one
level so the document reads as a single hierarchy. Nothing was rewritten,
condensed or reconciled during compilation. Edit the source files, not this one,
and recompile.

## Provenance

| section | source file | last commit | words |
|---|---|---|---|
| Abstract | `ABSTRACT_DRAFT_2026-08-23.md` | f0e0812 2026-08-26 | 854 |
| Methods | `METHODS_DRAFT_2026-08-19.md` | f0e0812 2026-08-26 | 20,038 |
| Results | `RESULTS_DRAFT_2026-08-23.md` | f0e0812 2026-08-26 | 4,502 |
| Discussion | `DISCUSSION_DRAFT_2026-08-23.md` | f0e0812 2026-08-26 | 2,676 |
| **total** | | | **28,070** |

## Status, honestly

**There is no Introduction.** No draft exists in the corpus. It is the one
manuscript section that has never been written, and it is not represented below.

**Abstract, Results and Discussion are clean prose** and are the mature parts of
the manuscript. The Abstract carries two versions (~300 words and ~150 words) plus
an editorial note on what was included and why.

**Methods is a working document, not manuscript prose.** At ~20,000 words it is
roughly four times a publishable methods section. It interleaves the method with
the reasoning behind it, the alternatives rejected, and warnings to a future
reader, much of which belongs in a supplement or nowhere. Read it as the record of
what was done; it will need to be cut and rewritten before submission, and that
work has not started.

Methods also covers **two tracks**: §2.1–§2.11 is the recombination and
calibration work, and §2.12 is the production analysis this paper reports. Both
are included here because both are cited by Results.

**Known open items inside Methods**, as of this compilation:

- `DRAFT_READINESS_2026-08-23.md` §2.1 lists eight §2.12 subsections as describing
  the wrong (88-unit) partition. That warning is now largely stale: a check on
  2026-08-26 found most of those passages are labelled derivation steps
  ("result before refinement", "the refinement was applied in the control run"),
  which is correct. §2.12.9 was genuinely wrong, gave the tally on the 88-unit
  basis, and was corrected to the reported 85-unit basis. §2.12.10's
  production/control designation was also genuinely wrong and was corrected.
  The remaining subsections should still be read with the basis in mind.
- §2.10 and §2.12.13 carry **two different software version tables** that disagree
  on IQ-TREE (2.4.0 vs 2.2.6) and SKA (0.5.0 vs 0.4.0). Unresolved; reconcile
  before submission.
- The control run's Nextflow version (25.10.0) is **still unverified** against the
  A100 host's log. Verify or drop it.

**Figures and tables are not included.** Table and figure content lives in
`MANUSCRIPT_OUTLINE_2026-08-21.md` and the `NUMBERS.tsv` key registry.

## The headline figures, for orientation

| quantity | value |
|---|---|
| panel | 2,959 genomes |
| analysed basis | 85 units, 2,340 genomes, 170 replicon-units |
| recombination, in-window | r/m **7.70** (median of 47 Gate 1 units) |
| validation set | **46 scorable genomes from 45 patients**, 16 exposure countries |
| country attribution | **10/46 (22%)** vs a 26% majority baseline, κ 0.193 |
| region attribution | **41/46 (89%)** vs a 46% baseline, κ 0.832 (modal k=20) |
| geography vs study of origin | **6 of 85** units carry geographic signal the BioProject control does not explain |
| reproducibility | re-run from the pin recovers Gate 1 = 47 units, median r/m 7.70 |

*(These are for orientation only. Every figure in the text carries its own
provenance tag; where this table and the text disagree, the text and its
`NUMBERS.tsv` key are authoritative.)*
...
> Source: `ABSTRACT_DRAFT_2026-08-23.md`, verbatim.
...
> **NOT WRITTEN.** No introduction draft exists in the corpus.
...
> Source: `METHODS_DRAFT_2026-08-19.md`, verbatim.
...
> Source: `RESULTS_DRAFT_2026-08-23.md`, verbatim.
...
> Source: `DISCUSSION_DRAFT_2026-08-23.md`, verbatim.
