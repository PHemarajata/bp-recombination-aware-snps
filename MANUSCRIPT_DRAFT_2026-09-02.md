...
> **[CONFIRM] Figure 5 disagrees with the text above it.** The script recomputes
> the null from `TIER2_null.txt` and gets **1,302 replicates over 54
> unit-replicons** and a separation of **434x to 2,131x**, against the **1,519**
> and **427x to 2,234x** stated in the Abstract and Results section 5. The
> upper figure reconciles if the greatest null r/m is 0.00668, which the table
> rounds to 0.007, so the two are probably the same run with the table reporting
> a subset. That must be established rather than assumed, and whichever is right,
> the Abstract, Results and figure must be made to agree before submission. The
> script prints the discrepancy on every run rather than absorbing it.

**iTOL components for Figure 3.** `make_itol_bp.py` writes `itol/`, containing the
pruned 85-tip tree and six iTOL datasets: readable labels, Gate 1 class and
dominant country as color strips, and r/m, diversity and unit size as bars. Same
basis and same guard as the figure, so a reviewer can be given the tree rather
than a picture of it. `itol/README.md` carries the same caveats as the Figure 3
caption, because a tree that can be re-styled is a tree that can be re-styled
