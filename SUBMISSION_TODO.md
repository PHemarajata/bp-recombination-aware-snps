...
| B2 | **Deposit the 312 in-house assemblies** (259 `IP-`, 53 `IE-`) | B1. All 312 currently carry `bioproject = unknown` or blank. **Now also a scientific dependency, not just administrative**: 276 of them are in the analysis basis and all would share one BioProject, which the control cannot presently see. Assigning them a synthetic identifier drops the geographic count from **6 to 5** and leaves only 2 units passing comfortably (`BIOPROJECT_COUNTERFACTUAL` §5). Depositing makes that assumption a fact and also makes the attribution leakage test possible, which is currently untestable at 13 of 46 |
