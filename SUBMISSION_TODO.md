...
| A2 | ✅ **DONE 2026-08-26** | Annotated tag **`v1.0.5-mod`** at `79ab645` (tip of `main`/`origin/main`), pushed to origin. Next in sequence after `v1.0.3-mod`/`v1.0.4-mod`, which point at 2025-10-07 commits. `DATA_AVAILABILITY_2026-08-24.md` now cites the release. ⚠ The manifest at `79ab645` still says `version = '1.0.3-mod'`, so run logs print that string while the `v1.0.3-mod` tag is a different, much older commit; left uncorrected on purpose, since bumping it would change the SHA the manuscript pins. Tag message also records that the commit predates the `gubbins_seed` fix. ⚠ **CORRECTED 2026-09-04: no such fix exists**, on any branch; the tag message is wrong and cannot be fixed without retagging a published release. **A GitHub Release object was not created**; do that as part of A1 if Zenodo archiving is wired to releases |
| A3 | ✅ **DONE 2026-09-01** | Recovered from the A100 host's `~/wf-assembly-snps-mod/.nextflow.log`, not reconstructed. Pinned verbatim in `PRODUCTION_RUN_PIN_2026-08-24.md` §7. **Every analysis parameter is identical to the reported run**, and both carry the `bp` profile, so the 50,000 Mash sketch applies to both. The two runs differ in exactly one analysis-relevant respect: the resource profile (`dgx_station_a100_updated` vs `local_workstation_rtx4070`). Methods §2.12.10 updated to say so rather than "different resource configurations". The invocation carries `-resume`, but **that is resolved**: `~/wf-assembly-snps-mod/.nextflow.log*` returns exactly one file, and Nextflow rotates prior sessions' logs, so no earlier session was ever launched from that directory. The pinned line is the *only* recorded invocation, not merely the last, and needs no "final session" qualifier. `-resume` was a no-op on a first run. ⚠ The `pipeline_info/` under `~/wf-assembly-snps-mod/` is dated **2026-05-28 and belongs to an unrelated run** — do not cite it for the control. **The control's own record survives** at `/data/scratch/v4c/L1v4c_out/pipeline_info/`, stamped `2026-08-19_11-49-18` against the log's `11:49:16` launch, so the control is now verified from **two independent sources, the same standard as the reported run**. Wall time ≈3 h 22 m (11:49 → 15:11). **Follow-on: A5** |
| A4 | ✅ **DONE 2026-09-01** | **Nextflow 25.10.0 confirmed** for the A100 control, read from the log's own banner line (`Aug-19 11:49:16 [main] INFO nextflow.cli.CmdRun - N E X T F L O W ~ version 25.10.0`). The "unverified" qualifier is removed from Methods §2.12.13. The guess was right, but it was a guess until now |
| A5 | ✅ **DONE 2026-09-01** | All five files pulled off the A100's unbacked-up scratch and held locally; they belong in the A1 archive. Findings, all written into `PRODUCTION_RUN_PIN_2026-08-24.md` §7.4 and Methods §2.12.10: **(a)** both runs record the same Nextflow `Script ID` `e09a5c4eadba2c5984f6790095423ee4`, so they **executed byte-identical pipeline code** — a stronger claim than a shared commit, since it hashes `main.nf` itself; **(b)** every containerised tool version matches (Gubbins 3.4.3, IQ-TREE 2.2.6, snippy 4.6.0, parsnp 1.7.4, numpy 1.26.2, Ubuntu 22.04.5), so the runs differ in exactly **two** respects, Nextflow version and resource profile; **(c)** both report **0 cached / 0 ignored / 0 failed / 0 retries**, which proves the control's `-resume` did no resuming and that neither run hid an `errorStrategy`-ignored failure of the kind that made `rc=0` misleading in D1; **(d)** the control ran **2,342 genomes to the reported run's 2,352**, a strict subset, and those 10 absent genomes are the root cause of the concordance-pairing defect in `A100_CFML_VALIDATION_2026-09-01.md` §4.1 |
| A6 | **Recover BioProject for the 150 public read runs that lack it** | Opened 2026-09-02 by the BioProject counterfactual. 426 of 2,340 analysis genomes have no usable BioProject; **150 are public SRR/ERR accessions and are a straightforward ENA lookup**, the other 276 are in-house and need B1/B2 instead. Worth doing because these are genuine archived facts that we simply have not fetched. ⚠ **Do not justify it as "can only reduce the count"** — that framing was retracted 2026-09-02: the control also *over*-controls, because BioProject is 95% single-country, so its errors run in both directions (`BIOPROJECT_COUNTERFACTUAL` §6). ⚠ Use **ENA, not NCBI efetch**, which has silently returned a different sample with HTTP 200 on SAMEA accessions here; always diff requested against returned. See `BIOPROJECT_COUNTERFACTUAL_2026-09-02.md` §1a and §4 |
...
| B2 | **Deposit the 312 in-house assemblies** (259 `IP-`, 53 `IE-`) | B1. All 312 currently carry `bioproject = unknown` or blank. 276 of them are in the analysis basis. ⚠ **Do NOT model them as one synthetic BioProject** — that was tried and rejected 2026-09-02 (`BIOPROJECT_COUNTERFACTUAL` §6): they are one province over one period, so their coherence is geographic rather than administrative, and treating it as batch removes real geography. Both units it flipped show no independent batch structure on the conditional test. Depositing still matters, because it makes the attribution leakage test possible, which is currently untestable at 13 of 46 |
