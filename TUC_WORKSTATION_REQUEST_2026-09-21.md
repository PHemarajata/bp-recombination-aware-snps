...
deck. It asks for seven values, three judgements and one missing source file.
It does **not** ask you to send any table.
...
## D. One file to bring back: the scene source that was never committed

This is the only ask in this document that wants a **file** rather than a value,
and it is source code with no data in it.

`Act4NotSeparable.mp4` is clip 4 act 6, 51.0 s. **No source for it exists on the
Mac, and none ever has in git history** anywhere in the repo. It is the *only*
orphan among all 34 delivered parts; every other part maps to a class in
`tuc_clip{1,2,3,4}_scenes.py` or `tuc_legacy_scenes_merged.py`.

It was not generated by some other tool. Its container metadata reads
`Rendered with Manim Community v0.21.0`, byte-identical to the metadata on parts
whose sources we do have, and its mtime sits in the same copy batch. So a `.py`
holding `class Act4NotSeparable(Scene)` existed on that workstation and was
simply never committed. The repo `.gitignore` denies by default, which is
exactly how a source file goes missing without anyone noticing.

Until it is recovered, **clip 4 cannot be fully re-rendered**, which blocks any
series-wide visual change: 51.0 s of 472.6 s, about 11 percent of the film.
...
# if that finds nothing: Manim writes to media/videos/<SOURCE_STEM>/1080p60/<Scene>.mp4,
# so the render output names the source file for you
find ~ -path '*/1080p60/Act4NotSeparable.mp4' 2>/dev/null

mdfind "Act4NotSeparable"

# THE LIKELY CAUSE: .gitignore denies by default, so an uncommitted source shows
# as ignored rather than untracked and never appears in a plain `git status`
cd ~/bp-recombination-aware-snps && git status --ignored --short | grep '\.py$'
```

Bring back:

- **D1.** the `.py` file itself, and anything local it imports.
- **D2.** the output of `git status --ignored --short` in that repo. If this
  source was never committed, something else may not have been either, and that
  is worth knowing on this trip rather than the next one.

**Read it before sending it.** It should be Manim scene code and nothing else.
If it contains an accession, a sample identifier or a path into a restricted
table, say so instead of pasting it, and we will handle it differently. The
remote is public.

If the file is genuinely gone, say so plainly and do not reconstruct it. The
delivered mp4 is a complete visual reference and rebuilding it here from
`BRIEF-04` is a known fallback.
...
D1  Act4NotSeparable source: FOUND at <path> | NOT FOUND
    (if found, paste the file separately after reading it)
D2  git status --ignored --short, .py lines only:
    <paste, or "nothing ignored">
```
