...
| ⚠ **Liang S, Mei S, Ji J, … Zhang H. DeepSANet. *Food Res Int* 2025;221(Pt 4):117554.** PMID **41185308** | **80.83% country accuracy** from 3,002 cgMLST loci, on *"a public S. Enteritidis dataset"* (almost certainly Bayliss's) | **Paywalled — methods NOT read; say so.** Two cautions before citing: (1) ⚠ **metric mismatch** — DeepSANet reports **accuracy**, Bayliss reports **macro F1**. On 38 imbalanced classes accuracy is dominated by well-sampled classes while macro F1 weights all equally, so **80.83% and 0.661 may not conflict at all**. Do NOT write "81% where Bayliss got 66%". (2) if it uses the Bayliss benchmark it **inherits the random stratified split**. Treat as an architecture comparison on a fixed benchmark, not evidence that country attribution generalises. **Action: obtain the PDF, read the splitting section** |
